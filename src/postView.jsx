    ...
    render()
    {
        return(
            <div style={{padding:10, backgroundColor:"#3d4553"}}>
                ...
                <a href={"http://php-empty-site-p7x3h7.azurewebsites.net/profile.html?user=" + this.props.user} style={{margin:0, display:"inline-block", fontSize:40, verticalAlign:"top", color:"#fcc229", paddingLeft:15, textDecoration:"none"}}>{this.props.username}</a>
            </div>
        )
    }
}
...
class Comments extends React.Component
{
    ...
    addComment()
    {
    	...
    	xhttp.open("GET", "http://php-empty-site-p7x3h7.azurewebsites.net/addComment.php?comment=\"" + this.textField.value + "\"&post=" + this.props.post + "&user=" + localStorage.getItem("userID"));
    	xhttp.send();
   ...
   loadComments(num)
   {
    ...
    loadComment()
    {
    	...
    	xhttp.open("GET", "http://php-empty-site-p7x3h7.azurewebsites.net/getCommentData.php?arg1=" + this.props.post + "&arg2=" + (this.props.comments - this.commentsLoaded));
    	xhttp.send();
    }
        ...
        this.state.comments.forEach((comment, key) => {
			...
			for (let i = 0; i < text.length; i++)
			{
				...
				if (word)
				{
					if (word.substr(0, 1) == "@")
					{
           	    	    tags.push(<a key={i} href={"http://php-empty-site-p7x3h7.azurewebsites.net/profile.html?username=" + word.substr(1)} style={{verticalAlign:"top", margin:0, color:"#e77225", paddingLeft:5, display:"inline-block", textDecoration:"none"}}>{word.substr(1)}</a>);
					}
					else
					{
               	...
               	comments.push(
            	    <div style={{padding:10}} key={key}>
						...
						<div style={{display:"inline-block", verticalAlign:"top", width:"calc(100% - 40px)"}}>
            	        	<a href={"http://php-empty-site-p7x3h7.azurewebsites.net/profile.html?user=" + comment.user} style={{paddingLeft:5, verticalAlign:"top", margin:0, display:"inline-block", color:"#fcc229", textDecoration:"none"}}>{comment.username}:</a>
							{tags}
						</div>
					</div>
            	);
...
class PostView extends React.Component
{
    ...
    refresh(cb)
    {
    	...
    	xhttp.open("GET", "http://php-empty-site-p7x3h7.azurewebsites.net/getPostData.php?arg1=" + this.props.user + "&arg2=" + this.props.post + "&arg3=" + localStorage.getItem("userID"), true);
    	xhttp.send();
    }
    ...
    addLike()
    {
    	...
    	xhttp.open("GET", "http://php-empty-site-p7x3h7.azurewebsites.net/addLike.php?user=" + localStorage.getItem("userID") + "&post=" + this.state.id, true);
    	xhttp.send();
    }
    ...
    removeLike()
    {
    	...
    	xhttp.open("GET", "http://php-empty-site-p7x3h7.azurewebsites.net/removeLike.php?user=" + localStorage.getItem("userID") + "&post=" + this.state.id, true);
    	xhttp.send();
    }
