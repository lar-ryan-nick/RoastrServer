...
if ($result = mysqli_query($conn, $sql)) 
{
	for ($i = 0; $i < $post && $row = mysqli_fetch_assoc($result); $i++)
    {
		...
		$postData = array('imageFilename' => $filename, 'profileFilename' => $filename2, 'id' => $row2['id'], 'user' => $row2['user'], 'timePosted' => $row2['timePosted'], 'caption' => $row2['caption'], 'image' => $image, 'username' => $row3['username'], 'likes' => mysqli_num_rows($result2) - 1, 'liked' => $liked, 'comments' => mysqli_num_rows($result), 'profilePicture' => $profilePicture);
